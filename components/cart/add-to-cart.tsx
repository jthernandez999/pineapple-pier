...
export function AddToCart({ product }: { product: Product }) {
   ...
   // This ensures that if no option is selected, we default to the first available size and color.
   let defaultState: { [key: string]: string } = {};
   if (variants.length > 0) {
      variants[0]?.selectedOptions.forEach((option) => {
         defaultState[option.name.toLowerCase()] = option.value;
      });
   }
   ...
   // Find the matching variant using the merged state.
   const variant = variants.find((variant: ProductVariant) =>
      variant.selectedOptions.every(
         (option) => option.value === mergedState[option.name.toLowerCase()]
      )
   );
