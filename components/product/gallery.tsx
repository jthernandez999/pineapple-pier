...
export function Gallery({ images }: { images: { src: string; altText: string }[] }) {
   ...
   // Normalize a string: trim, uppercase, and remove spaces and hyphens.
   const normalize = (s: string) => s.trim().toUpperCase().replace(/[\s-]/g, '');

   // Use alt text for filtering images based on the color.
   const selectedColor = state.color ? normalize(state.color) : '';
   const filteredImages = selectedColor
      ? images.filter((img) => {
           // Split the alt text by '-' and compare the first part to the selected color.
           const parts = img.altText.split('-');
           return parts.length > 0 && normalize(parts[0] as string) === selectedColor;
        })
      : images;

   // Fallback to the full images array if filtering yields nothing.
   const effectiveImages = filteredImages.length > 0 ? filteredImages : images;

   // imageIndex is derived from the state.image (a numeric index stored as a string).
   const imageIndex = state.image ? parseInt(state.image) : 0;

   // Calculate navigation indexes based solely on effectiveImages.
   ...
   return (
      <form>
         ...
         {/* Thumbnails: display all effectiveImages as thumbnails */}
         {effectiveImages.length > 1 && (
